...
calcGradient <- function(datalist, mem.avail.vec, resids, types){
  ...
  for(i in 1:length(types)){
    thistype <- types[i]
    if (thistype=="category"){
      gradient[[i]] <- gradCat(datalist[[i]], mem.avail.vec[i], resids)
    } else if (thistype=="linear"){
      gradient[[i]] <- gradLin(datalist[[i]], resids)
    } else if (thistype=="smooth"){
      gradient[[i]] <- resids 
    } else if (thistype=="textfile" | thistype=="textstring"){
      gradient[[i]] <- gradText(datalist[[i]], mem.avail.vec[i], resids)
    } 
  }
  return(gradient)
}
